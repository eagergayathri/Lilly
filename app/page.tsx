...
const SCORES = [
  { 
    ...
    logic: '80% markdowns confirmed on 400+ items. Lowest floor in 6 months for Brampton area.' 
  },
  { 
    id: 2, 
    brand: 'GAP', 
    ...
    logic: 'Extra 50% off markdowns applied. Integrity check confirms widespread .97 price endings.' 
  },
  { 
    id: 3, 
    brand: 'H&M', 
...
export default function LillyApp() {
  const [view, setView] = useState('dashboard'); // 'dashboard' or 'about'
  ...
  return (
    <main style={{ backgroundColor: '#050505', minHeight: '100vh', color: 'white', padding: '24px', fontFamily: 'sans-serif' }}>
      <style>{`
        ...
        .nav-btn { background: none; border: none; color: #444; font-[9px]; font-weight: 900; text-transform: uppercase; letter-spacing: 2px; cursor: pointer; padding: 8px; }
        ...
        .about-text { color: #888; font-size: 13px; lineHeight: 1.6; margin-bottom: 24px; }
      `}</style>
      ...
      {view === 'dashboard' ? (
        ...
        <div style={{ animation: 'fadeIn 0.3s ease' }}>
          ...
          <p className="about-text">
            Lilly isn't a scraper; she's an <strong>Integrity Engine</strong>. The scores are generated by cross-referencing live inventory data with 13 years of retail pricing patterns in the GTA region.
          </p>

          <span className="about-title">The .97 Rule</span>
          <p className="about-text">
            In retail governance, prices ending in <strong>.97</strong> are "hard-coded" clearance. These items will never be cheaper. When Lilly detects a .97 density above 40%, the brand's score spikes to <strong>ACT NOW</strong>.
          </p>

          <span className="about-title">Market Sentiment</span>
          <p className="about-text">
            Lilly looks at "Brand Velocity." If H&M is pushing 20% promos but still has high stock levels, she flags it as a <strong>WAIT</strong>, predicting a deeper 50% drop within 14-21 days.
          </p>

          <span className="about-title">Decision Matrix</span>
          <div style={{ background: '#111', padding: '20px', borderRadius: '16px', border: '1px border #1a1a1a' }}>
      ...
      <footer style={{ marginTop: '40px', textAlign: 'center', opacity: 0.2 }}>
        <p style={{ fontSize: '8px', letterSpacing: '4px' }}>LILLY ENGINE V1.5</p>
      </footer>
    </main>
  );
}
